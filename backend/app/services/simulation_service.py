...
class SimulationService:
    ...
    def create_simulation(req: schemas.SimulationRequest) -> str:
        ...
        model_type = req.model_type or "DELFT3D"
        if model_type == "DUAL":
            for m in ["SPH", "DELFT3D"]:
                adapter = ModelRegistry.get_adapter(m)
                if not adapter.is_available:
                    raise ValueError(f"Model {m} runtime is unavailable for DUAL comparison")
        else:
            try:
